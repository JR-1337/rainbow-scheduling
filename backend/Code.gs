...
// ═══════════════════════════════════════════════════════════════════════════════
// TEST-EMPLOYEE PURGE (admin-only, run from Apps Script editor)
// Identifies accounts with @example.com emails and removes them across
// Employees + Shifts + ShiftChanges. listTestEmployees is read-only;
// purgeTestEmployees is destructive. Run list first, review log, then purge.
// ═══════════════════════════════════════════════════════════════════════════════

function _findTestEmployees_() {
  const ss = getSpreadsheet();
  const empSheet = ss.getSheetByName(CONFIG.TABS.EMPLOYEES);
  ...
  const matches = [];
  for (let i = 1; i < values.length; i++) {
    const email = String(values[i][emailIdx] || '').toLowerCase();
    if (email.indexOf('@example.com') !== -1) {
      matches.push({
        rowIndex: i + 1,
        id: String(values[i][idIdx]),
        name: String(values[i][nameIdx]),
        email: String(values[i][emailIdx]),
        active: values[i][activeIdx]
      });
    }
  }
  return matches;
...
function listTestEmployees() {
  const matches = _findTestEmployees_();
  Logger.log('=== Test-employee candidates (@example.com) ===');
  Logger.log('Count: ' + matches.length);
  matches.forEach(m => Logger.log('  id=' + m.id + '  ' + m.name + '  ' + m.email + '  active=' + m.active));

  if (matches.length === 0) {
    Logger.log('Nothing to purge.');
    return;
  }

  const ss = getSpreadsheet();
  const testEmails = matches.map(m => String(m.email).toLowerCase());
  const testIds = matches.map(m => String(m.id));

  // Shifts: match employeeEmail OR employeeId
  const shiftsSheet = ss.getSheetByName(CONFIG.TABS.SHIFTS);
  const sv = shiftsSheet.getDataRange().getValues();
  const sh = sv[0].map(String);
  const sEmpId = sh.indexOf('employeeId');
  const sEmail = sh.indexOf('employeeEmail');
  let shiftMatches = 0;
  for (let i = 1; i < sv.length; i++) {
    const e = String(sv[i][sEmail] || '').toLowerCase();
    const id = String(sv[i][sEmpId] || '');
    if (testEmails.indexOf(e) !== -1 || testIds.indexOf(id) !== -1) shiftMatches++;
  }
  Logger.log('Shifts rows matching: ' + shiftMatches);

  // ShiftChanges: match employeeEmail, recipientEmail, or partnerEmail
  const changesSheet = ss.getSheetByName(CONFIG.TABS.SHIFT_CHANGES);
  if (changesSheet && changesSheet.getLastRow() > 1) {
    const cv = changesSheet.getDataRange().getValues();
    const ch = cv[0].map(String);
    const cEmail = ch.indexOf('employeeEmail');
    const cRecip = ch.indexOf('recipientEmail');
    const cPartner = ch.indexOf('partnerEmail');
    let changeMatches = 0;
    for (let i = 1; i < cv.length; i++) {
      const e = String(cv[i][cEmail] || '').toLowerCase();
      const r = cEmail === -1 ? '' : String(cv[i][cRecip] || '').toLowerCase();
      const p = cPartner === -1 ? '' : String(cv[i][cPartner] || '').toLowerCase();
      if (testEmails.indexOf(e) !== -1 || testEmails.indexOf(r) !== -1 || testEmails.indexOf(p) !== -1) changeMatches++;
    }
    Logger.log('ShiftChanges rows matching: ' + changeMatches);
  } else {
    Logger.log('ShiftChanges rows matching: 0 (sheet empty or missing)');
  }

  Logger.log('');
  Logger.log('To actually delete: run purgeTestEmployees()');
}

function purgeTestEmployees() {
  const matches = _findTestEmployees_();
  if (matches.length === 0) { Logger.log('No @example.com employees found.'); return; }

  const ss = getSpreadsheet();
  const testEmails = matches.map(m => String(m.email).toLowerCase());
  const testIds = matches.map(m => String(m.id));
  const summary = { employees: 0, shifts: 0, changes: 0 };

  // Delete bottom-up so row indices don't shift.
  const deleteMatching = (sheet, predicate, label) => {
    if (!sheet || sheet.getLastRow() <= 1) return 0;
    const values = sheet.getDataRange().getValues();
    const headers = values[0].map(String);
    let n = 0;
    for (let i = values.length - 1; i >= 1; i--) {
      if (predicate(values[i], headers)) {
        sheet.deleteRow(i + 1);
        n++;
      }
    }
    Logger.log('Deleted ' + n + ' rows from ' + label);
    return n;
  };

  const shiftsSheet = ss.getSheetByName(CONFIG.TABS.SHIFTS);
  summary.shifts = deleteMatching(shiftsSheet, (row, h) => {
    const e = String(row[h.indexOf('employeeEmail')] || '').toLowerCase();
    const id = String(row[h.indexOf('employeeId')] || '');
    return testEmails.indexOf(e) !== -1 || testIds.indexOf(id) !== -1;
  }, 'Shifts');

  const changesSheet = ss.getSheetByName(CONFIG.TABS.SHIFT_CHANGES);
  summary.changes = deleteMatching(changesSheet, (row, h) => {
    const cEmail = h.indexOf('employeeEmail');
    const cRecip = h.indexOf('recipientEmail');
    const cPartner = h.indexOf('partnerEmail');
    const e = String(row[cEmail] || '').toLowerCase();
    const r = cRecip === -1 ? '' : String(row[cRecip] || '').toLowerCase();
    const p = cPartner === -1 ? '' : String(row[cPartner] || '').toLowerCase();
    return testEmails.indexOf(e) !== -1 || testEmails.indexOf(r) !== -1 || testEmails.indexOf(p) !== -1;
  }, 'ShiftChanges');

  const empSheet = ss.getSheetByName(CONFIG.TABS.EMPLOYEES);
  summary.employees = deleteMatching(empSheet, (row, h) => {
    const e = String(row[h.indexOf('email')] || '').toLowerCase();
    return e.indexOf('@example.com') !== -1;
  }, 'Employees');

  Logger.log('');
  Logger.log('=== Purge complete ===');
  Logger.log('Employees removed: ' + summary.employees);
  Logger.log('Shifts rows removed: ' + summary.shifts);
  Logger.log('ShiftChanges rows removed: ' + summary.changes);
}
