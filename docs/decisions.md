...
## 2026-04-14 - PK bulk modal: auto-include by real availability-window overlap (S63)
**Decided:** `PKEventModal` attendee auto-check logic: every active employee whose `availability[weekday].available !== false` AND whose `[start, end]` window encloses the PK window is checked by default. Employees failing either test appear in the list greyed out with a reason ("unavailable wed" / "avail 11:00-18:00 only"). Admin can click any row to override on or off. Default PK window 18:00-20:00; admin can adjust, attendee list re-computes. Implemented via new `availabilityCoversWindow(availability, dateStr, startHHMM, endHHMM)` pure helper in `src/utils/timemath.js`.
**Alternatives:** (a) Auto-check ALL active employees, ignore availability entirely — rejected. Staff with `available: false` on that day (explicit days-off) would be scheduled into mandatory training. (b) Only hard-exclude on the day-level `available === false` flag, ignore start/end times — briefly shipped mid-session as a reaction to a stale handoff note claiming "availability start/end are default work times, not real free/busy." JR overruled: those times ARE real availability; prior claim came from a Sarvi misunderstanding, not a real constraint. Restored the time-window overlap check. (c) Hide ineligible rows from the list — rejected. Greyed visibility with reason keeps the admin informed and lets her force-include if she knows something the data doesn't (overtime volunteer, special case).
**Rationale:** Time-window check flags real scheduling conflicts before they happen — a staffer whose availability ends at 6pm physically can't attend a 6-8pm PK. Soft exclusion (greyed + clickable) preserves admin agency.
**Revisit if:** Sarvi reports false exclusions (someone who should be eligible showing greyed), which would indicate the `availability` data is not what we think it is. Re-check JR's stated semantics before assuming the helper is wrong.

## 2026-04-14 - Autofill toolbar collapse: 4 controls → 3 via dropdown top option (S63)
**Decided:** The schedule-edit-mode autofill toolbar at `src/App.jsx:3428-3507` collapses from 4 sibling controls (Auto-Fill All FT button + Auto-Fill Week dropdown + Clear Week dropdown + Clear All FT button) to 3 (Auto-Fill dropdown + Clear dropdown + Schedule PK button). Each dropdown's top option is bold "All Full-Timers" which triggers the bulk action; remaining options inside `<optgroup label="— or pick one —">` trigger the per-employee action. The onChange handler branches on `value === '__all__'` for the bulk path.
**Alternatives:** (a) Keep 4 buttons (status quo) — rejected per JR: "that looks redundant and messy." (b) Custom dropdown component with richer styling per option — rejected as scope creep; native `<select>` + `<optgroup>` separator is cross-browser, accessible, zero new component.
**Rationale:** Standalone buttons duplicated what a dropdown's first option can express. Fewer controls + consistent interaction pattern (pick from dropdown) + visual hierarchy via bold + optgroup separator.
**Revisit if:** JR later wants per-employee autofill more discoverable than a dropdown option (e.g., autocomplete input), OR the PK button needs a sibling event-scheduling action (e.g., bulk meeting).
