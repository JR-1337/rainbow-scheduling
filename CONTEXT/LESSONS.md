...
Affirmations: 0
dropped: provenance (no commit hash recoverable for this 2026-04-19 fix)
...
Affirmations: 0
dropped: provenance (no commit hash recoverable for Bug 5 investigation)
...
Affirmations: 0
dropped: provenance (no commit hash recoverable for the PK/MTG badge relocation)
...
Provenance: 2026-04-19 -- <unknown commit> -- auto-populate confirm `<Modal>` lived only in the desktop return; mobile Clear Wk / Fill Wk-with-existing / PK-autofill-confirm dispatched `setAutoPopulateConfirm({...})` but nothing mounted; Fix 1 extracted JSX into a shared const.
Tags: surface: react, concern: render
Affirmations: 0
dropped: provenance (no commit hash recoverable for Fix 1 ship 2026-04-19)
...
Tags: surface: react, concern: ux
Affirmations: 0
dropped: provenance (no date or commit hash in source)
...
Tags: surface: react, concern: render
Affirmations: 0
dropped: provenance (no commit hash recoverable for 2026-04-18 re-verify observation)
...
Rule: Compare Sheets boolean columns with `=== true` / `=== false`, never truthy/falsy.
Trigger: When reading `isOwner`, `isAdmin`, `active`, or `deleted` from the Employees tab.
Why: Sheets returns these as the strings "TRUE" / "FALSE", which are both truthy. Truthy/falsy checks evaluate "FALSE" as true and silently invert auth/visibility.
Provenance: <unknown commit> -- Sheets boolean-as-string pitfall on Employees tab.
Tags: surface: sheets, concern: type-coercion
Affirmations: 0
dropped: provenance (no date or commit hash in source)
...
Affirmations: 0
dropped: provenance (no date or commit hash for v2.19 attempt)
...
Affirmations: 0
dropped: provenance (no commit hash for the research doc)
...
Why: JR stated 2026-04-23 "opus 4.7 plans the code and does the system 2 thinking and then a sonnet 4.6 sub agent executes the very specific plan." Reaffirmed 2026-04-25 during /coding-plan skill design after seeing per-run cost numbers (~$1.50-2.50, 80% in Opus planning).
...
Why: ESA is real but over-reliance signals monomaniacal framing. JR forbade ESA mentions on Slide 4 Alternatives.
...
Why: SaaS-hero register clashes with the "high-priced lawyer with charm" voice JR specified. The phrases above are the recurring offenders.
...
Rule: Use only Sarvi-confirmed facts (35 staff, 14 hr/wk, $30,452/yr current-state cost), statutory facts (O. Reg. 189/24 $5K), or pure arithmetic from those; range with "roughly" if not sourced.
...
Why: Audience is family and Amy distrusts automation; Sarvi-at-risk framing reads as adversarial. Process-scale angles land better with the same audience.
...
Rule: Use the 14-hour full-admin figure (schedules + management talks + time-off + swaps + sick calls + push + off-hours) in ROI math, not grid hours alone, and avoid the word "envelope" in customer-facing copy (use "scheduling work" or "the time Sarvi spends on the schedule").
Trigger: When writing Slide 2 framing, chatbot FACT 3, or any ROI math.
Why: Grid-only hours undercount the real cost; "envelope" reads as opaque jargon and JR flagged it in s027 sweep.
...
Why: The plan is the contract. Skipping the re-read drove three rounds of rework ("I don't think ur following the plan").
...
Rule: Pixel-detect modal bounds with Python PIL luminance-jump from each edge bracket on first index where `(r+g+b)/3 > 190` exceeds N; do not guess ffmpeg offsets.
...
Why: Inline `fontSize` overrides break hierarchy if any rung is skipped. S60 TV-viewing bump left card fonts larger than the subheader.
...
Rule: Write pitch-deck copy and chatbot responses in a charming, force-of-logic trial-lawyer voice; first sentence is the answer; ban opening preamble ("That's a fair question", "Great question", "And the honest answer is"), banned phrases ("let's prove it together", "we've got you covered", "earns your trust"/"earns the trust", "infinitely better", "crush", "destroy", "annihilate"), emoji peppering, and more than one exclamation mark per response.
Trigger: When writing or editing any pitch-deck or chatbot response copy.
Why: Soft openers, banned phrases, and emoji peppering break the trial-lawyer voice. Read aloud: opens with a softener, lacks a metaphor, or spans more than 2 punchy paragraphs means rewrite.
...
Why: Family (Joel/Amy/Dan/Scott) does NOT cover missing staff; Sarvi absorbs all crisis recovery alone. Family-firm risk-aversion is fair, but family-firm operational labor is not.
Provenance: s024 -- Ripple "FAMILY DESK" framing rejected ("they sit on their hands and sarvi does it"); reframed to "INVISIBLE BILL" landing cost on business.
Tags: surface: prompt-kit, concern: naming
Affirmations: 0

## [PROJECT] -- Don't name JR personally in pitch DECK slides; spec/price body + footer + chatbot exempt
Rule: In pitch-deck slide copy (`src/slides/*.jsx` in RAINBOW-PITCH), refer to the product builder as "Rainbow" or "the developer", never "John"; allow named exemptions in Spec.jsx body (continuity sections), Spec.jsx + Price.jsx footer attributions ("Prepared by John Richmond"), and the chatbot system prompt in `api/ask-rainbow.js` (framed as "trial lawyer for John Richmond").
Trigger: When editing pitch-deck slide copy or any spec/price/chatbot artifact that might name the builder.
Why: Naming JR in slide copy reads as single-point-of-failure risk in family-firm sales context. Scoped exemptions preserve continuity context without exposing it on the deck.
Provenance: s025 -- Proposal Card 3 + Phase2 footer; s026 Spec.jsx alignment respected scope (John only in continuity body + footer + chatbot).
Tags: surface: prompt-kit, concern: naming
Affirmations: 1
