...
- **In-app bug fixes (JR to enumerate)** -- pause point before chatbot query-capture wiring; JR to surface the bug list next.
- **Wire up chatbot query capture (Apps Script -> Google Sheet).** Append each `/api/ask-rainbow` POST as a row (timestamp, truncated IP, question, answer length, latency, provider). ~15 lines added to `api/ask-rainbow.js` + ~10 lines Apps Script. Fire-and-forget sink. Deferred since s024. Hold until JR's bug list lands.
