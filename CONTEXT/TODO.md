...
- **Email logic + schedule distribution wiring (next-session priority)** -- JR has created a new OTR-dedicated Gmail to replace his personal account as the schedule-distribution sender. Unblocks the long-standing Email upgrade item (PDF auto-attached via MailApp; blocked since 2026-02-10). Next: audit current sender-config sites, wire new account, smoke a test send. Sender email value still pending from JR.
