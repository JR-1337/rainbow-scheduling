...
- Tablet white screen -- next: JR repro (which tablet, which URL, any console errors); could be viewport-range bug between mobile/desktop cutoffs
- Sarvi mobile cannot see new roles when scheduling -- next: JR/Sarvi repro (which role picker, which screen); likely ShiftEditorModal role chips missing on mobile viewport
...
- RISK: Apps Script v2.25.0 needs redeploy for schedule-change notifications to fire; until then non-Sarvi/non-JR admin edits stay silent
...
- [2026-04-19] Per-day defaults decouple + availability widening shipped (`576784b`, `176a371`, `16c7966`, `4b33f7e`, `1c360ca`, `9a02ad3`, `df161d9`, `3460c69`). Employees col N `defaultShift` (JSON per-day `{start,end}`); Auto-Fill's `createShiftFromAvailability` prefers defaultShift, falls back to availability; `availability.available` stays the gate. EmployeeFormModal has a new Default Shift grid below Availability with Clear per day and auto-seeded pair. One-shot editor-only `widenAvailabilityForPK_()` ran on live sheet 2026-04-19: scanned 24, changedEmployees 20, changedDayEntries 94. Dan + Scott skipped (no availability stored). New-hire form default availability widened to Mon-Fri 10:00-20:00, Sat 10:00-19:00, Sun store hours so future hires are PK-eligible without re-running the widener. Apps Script v2.24.0 deployed. Widener + `runWidenAvailabilityForPK` wrapper removed from repo per one-shot hygiene.
- [2026-04-19] PK flow cleanup shipped (`1b78bd2`, `9cce775`, `4fdb4da`). Single "Schedule PK" button replaces the dropdown desktop + mobile; `handleAutofillPKWeek` + its `autofill-pk-week` branches deleted; new `Saturday (MM-DD) · 10:00-10:45` quick-pick inside PKEventModal (reads `activeWeek` + `week1`/`week2` props). Follow-up polish: Select eligible button shows `(N)` count and disables at 0 (root cause: weekday default 18:00-20:00 fits zero OTR availabilities, so function had no rows to tick); mobile date/time row switched to `grid-cols-1 sm:grid-cols-3` so the 6 controls stop piling up at ~500px. Plan approved 2026-04-19 to decouple default booked hours from availability (`~/.claude/plans/per-day-defaults-are-humming-quokka.md`), to execute in a fresh chat.
- [2026-04-19] Mobile admin parity + PK clarity shipped (`13eac1b`, `eac7221`, `58be6fe`, `b2ce3e3`, `5ee7e74`). Fix 1: auto-populate confirm Modal extracted to shared `confirmModal` const rendered from both mobile and desktop returns (was desktop-only; mobile Clear/Fill/PK-autofill confirms never mounted). Fix 2: `clearWeekShifts` now also deletes from `events` so PKs/meetings are cleared alongside shifts. Fix 3: `handleAutofillPKWeek` toast names the resolved time window (e.g. "18:00-20:00") when zero eligible and points at Schedule-a-PK; partial days say "N day(s) had no eligible staff". Fix 4: `MobileAdminView` cell event badges stack vertically up to 2 (shortLabel + start time) and aggregate to "N events" at 3+. Fix 5: new `src/components/MobileScheduleActionSheet.jsx` (two-level AdaptiveModal bottom-sheet: root -> fill/clear/pk -> "All" + per-employee picker); replaced the three bare mobile buttons with a single Actions button. Build PASS at HEAD `5ee7e74` (~467.83 kB). Prod bundle `index-BXfrL2ua.js` deployed. JR phone-smoke pending. New LESSON saved: "Render shared overlays at App root, not inside branch returns".
