...
- **iPhone 12 / iOS Safari freeze investigation (Sarvi-reported).** Symptom: page visible but taps dead, exit + re-enter Safari tab fixes it, often after Save. Diagnosis points to synchronous render chain on Save (490-cell rebuild + `allViolations` recompute + `AnimatedNumber` rAF storm) blocking main thread on iPhone 12. Fix candidates ranked at `docs/iphone12-freeze-fixes-2026-05-06.md` (A1-A6 render cost, B1-B3 bundle weight, C1-C4 quality-of-life, D1 diagnostic). **Shipped:** Batch 1 A1+A2+A3 (`c386cb9`) -- useMemo week1/week2, EMPTY_EVENTS frozen constant on the three render paths missing it, today hoist out of per-cell loop. Batch 2 A4 (`c83c1fe`) -- AnimatedNumber rAF chain dropped on EmployeeRow + ColumnHeaderCell hot paths; modal AnimatedNumber preserved. **Next:** JR re-evaluates whether to push for Sarvi to retest, or pick A5/A6/B-tier from the list before deploy.

- **Autofill should skip holidays.** Auto-fill week currently fills shifts on stat-holiday dates. Want: skip holidays in the autofill pass. Detection helper `isStatHoliday` already exists in `src/utils/storeHours.js`. Scope: identify autofill logic call site, gate the fill loop on `!isStatHoliday(date)`. Confirm with JR whether holiday cells should also be visually flagged (e.g. greyed) in autofill preview so user knows they were intentionally skipped.
