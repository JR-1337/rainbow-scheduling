...
- **iPhone 12 / iOS Safari freeze investigation (Sarvi-reported).** Symptom: page visible but taps dead, exit + re-enter Safari tab fixes it, often after Save. Diagnosis points to synchronous render chain on Save (490-cell rebuild + `allViolations` recompute + `AnimatedNumber` rAF storm) blocking main thread on iPhone 12. Fix candidates ranked at `docs/iphone12-freeze-fixes-2026-05-06.md` (A1-A6 render cost, B1-B3 bundle weight, C1-C4 quality-of-life, D1 diagnostic). **Batch 1 (A1+A2+A3) shipped this commit:** useMemo week1/week2, EMPTY_EVENTS frozen constant on the three render paths missing it, today hoist out of per-cell loop in EmployeeRow. **Next:** A4 alone (kill AnimatedNumber rAF storm), then re-evaluate. No deploy until A4 lands.
