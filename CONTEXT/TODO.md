...
- **iPhone 12 / iOS Safari freeze investigation (Sarvi-reported).** Symptom (initial): page visible but taps dead, exit + re-enter Safari tab fixes it, often after Save. Symptom (follow-up): scrolling on the main schedule grid also freezes. Fix candidates ranked at `docs/iphone12-freeze-fixes-2026-05-06.md`. **Shipped:** Batch 1 A1+A2+A3 (`c386cb9`) -- useMemo week1/week2, EMPTY_EVENTS frozen constant, today hoist. Batch 2 A4 (`c83c1fe`) -- AnimatedNumber rAF chain dropped on hot paths. Batch 3 A5+B2+C3 (`c7c3406`) -- useDeferredValue on shifts/events for allViolations, Vite manualChunks vendor split (95 kB app + 45 kB cached React vs 140 kB combined), PDF blob URL retention 10min->30s. Batch 4 long-press scroll fix -- dropped non-passive touchmove listener from `useLongPress` (490 mobile cells were each attaching one, blocking iOS Safari scroll engagement); React onTouchMove + 28px coarse threshold still cancels timer on movement, long-press behavior unchanged. **Remaining candidates:** A6 (memoize mobile grids, ~2-3hr), B1 (lazy 14 modals, ~1-2hr), B3 (mobile/desktop view split, bigger refactor), C2 (useToast cleanup, ~10min). C1 skipped per JR. **Next:** wait for Sarvi feedback on whether scroll + Save freezes are cured.
