...
- Adversarial audit Phase E -- next: App.jsx extraction (multi-session, own branch); JR must redeploy `backend/Code.gs` v2.23.0 to live Apps Script (paste-over + new deployment) so hash-only auth takes effect. (Sub-areas 1, 3, 5, 3-extract-date-utils all DONE.)
