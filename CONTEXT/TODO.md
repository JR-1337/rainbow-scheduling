...
- Item 3 (Admin tier 2 + job title) -- shipped end-to-end including admin1 inheriting title (28cf429, 693696f, 6b59176, 5fece50, 303e4c5); JR phone-smoke pending
...
- Verify `5fece50` showOnSchedule preservation across tier changes (admin1 toggle no longer hides from grid)
...
- Perf + professional-app audit -- (a) read-only findings landed 2026-04-25 at `docs/perf-audit-app-jsx-2026-04-25.md` (7 findings: 2 HIGH ColumnHeader+ScheduleCell-styles, 3 MED PDF-lazy+getScheduledCount+EmployeeFormModal, 2 LOW); next: JR triage which to fix; (b) evaluate database + hosting upgrades beyond Sheets+AppsScript (Supabase/Postgres, Neon, Vercel Postgres) for professional security posture + auth + performance if OTR decides to buy the app
