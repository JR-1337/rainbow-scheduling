...
- **[s058 raised by JR] Sadie hard-deleted from Employees sheet (she quit, not coming back) -- her shifts still render on the schedule.** JR manually deleted Sadie's whole row from the Employees tab (`Sadie Kromm`, id `emp-1776186167048` per the prior `parentId` snapshot). She quit; no restore intended. Her shifts are still rendering on the schedule grid because Shifts rows carry denormalized `employeeId / employeeName / employeeEmail` columns and the grid renders from the cached shift data even with the Employees row gone. **Immediate cleanup for Sadie:** delete her future shift rows from the Shifts tab in the Sheet UI (filter by `employeeName=Sadie Kromm` or `employeeId=emp-1776186167048`); past shifts stay for payroll/history. **Broader design question for the same session:** (1) does the grid show stale name + email, or throw / "unknown employee" / silently filter? (2) data-integrity exposure -- can Sarvi accidentally hard-delete an employee with future shifts and create ghost rows that confuse payroll? Decide whether to add a defensive filter (skip shifts whose `employeeId` isn't in the active employees array) or surface an admin-side warning before any hard-delete. The parked **Employees archive sheet w/ 5-year retention** Active item below is the durable fix (snapshot emp name+id at delete-time, soft-delete only via `Erase` action); the defensive filter is the interim guard.
...
- **Apps Script live = `c817f07` (v2.31.1 paste-deployed 2026-05-03).** Onboarding email backend stack live: `sendOnboardingEmail` action + `backfillOnboardingDates` one-shot (ran, N=36 active employees marked) + `WELCOME_TEMPLATE_HTML_` + `LAUNCH_LIVE_=false` recipient gate + 6 brand/onboarding constants + v2.31.1 body-wrap (typed bodies now wrapped in `BRANDED_EMAIL_WRAPPER_HTML_`). v2.30.2 refactor (commit `23ad13b`) live. All prior audit-fix backend stack still live.
