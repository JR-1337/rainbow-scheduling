...
- **Last validated: s066 PDF attachment Phase 0 probe 2026-05-05 at HEAD `1ac1052`.** Apps Script HTML->PDF conversion via `Utilities.newBlob(html, 'text/html').getAs('application/pdf')` confirmed acceptable fidelity for the kitchen-door schedule from JR eyeball comparison vs the on-screen print preview. Backend v2.32.4 source paste-deployed. Frontend `1ac1052` pushed; Vercel auto-deploy in flight. Phase 4 smoke (group send + individual send + Test Guy reactivation/deactivation cycle) deferred to next session.
- Missing validation: Phase 4 smoke for PDF attachment -- group send to JR + individual send to Test Guy still owed.

- **Apps Script live = v2.32.4 (source paste-deployed 2026-05-05).** sendBrandedScheduleEmail now accepts optional `payload.pdfHtml` + `payload.pdfFilename`; converts HTML to PDF via Drive's HTML->PDF service (`Utilities.newBlob(html, 'text/html').getAs('application/pdf')`) and attaches to the MailApp send. Backwards-compatible: payload without pdfHtml behaves as before. Prior backend stack still live: v2.32.3 onboarding-BCC drop, v2.32.2 archive auto-clear, v2.32.1 audit fixes (`parseLocalDate_` + `canEditShiftDate_` timezone-correct + `!!` truthy isOwner; idempotent archiveEmployee/unarchiveEmployee/hardDeleteArchivedEmployee with deleteWarning surfacing; saveEmployee silent-strips owner-only no-op fields for non-owner), v2.32.0 past-period edit lock + EmployeesArchive sheet (`pastPeriodGraceDays` column on Employees, Sarvi's row=7), v2.31.1 onboarding email + welcome PDF, v2.30.2 batchSaveShifts withDocumentLock_, all prior audit-fix backend.
...
- [2026-05-06] **Employees UI -- Deleted tab removed; Archive chip + AdaptiveModal archive sheet (commit `fbb8568`).** Desktop `EmployeesPanel` + mobile `MobileStaffPanel` (title Staff): filters Active / Inactive / third chip Archive (`FolderArchive` icon). Chip visible to all admins; `openArchivedEmployeesPanel` opens `ArchivedEmployeesPanel` for owner only, else toast. Duplicate desktop admin menu row Archive... removed. Legacy `deleted` Employees-sheet rows list under Inactive with "(legacy removed)"; Archive action suppressed on those rows. `ArchivedEmployeesPanel` uses `AdaptiveModal` (bottom sheet on narrow viewports).

- [2026-05-06] **Weekly violation hours -- Monday-start week inside pay period (commit `0b83189`).** `allViolations` supplies net hours per Monday-start calendar week overlapping the active 14-day period (`src/utils/date.js`), not a single sum across both weeks. Shift editor splits `getEmpHoursFullPeriod` vs `getEmpHoursForWeekContaining`. `package.json` dev script uses `vite --host` for Chrome OS LAN testing.

- [2026-05-05] **s066 -- EmailModal v2 PDF attachment shipped (commit `1ac1052` scheduling app + `0a289d2` rainbow-pitch Ripple p3 footer rewrite).** Frontend extracts pure `buildScheduleHtml` builder from `generateSchedulePDF` (zero behavior change to existing print preview); EmailModal builds the same HTML once and POSTs `pdfHtml` + `pdfFilename` on group + individual sends (dynamic-imported to keep PDF code in its own chunk). App.jsx EmailModal call sites now pass `events` + `timeOffRequests` so the attached PDF renders meeting/PK/sick badges + OFF cells (desktop call site was missing `events` pre-existing). Backend v2.32.4 paste-deployed: `sendBrandedScheduleEmail` accepts optional pdfHtml + pdfFilename, converts via `Utilities.newBlob(html, 'text/html').getAs('application/pdf')`, attaches to MailApp send; backwards-compatible (payload without pdfHtml behaves as before). Phase 0 fidelity probe (`backend/pdf-probe.gs`, untracked, 217KB inlined real Wk19/20 schedule HTML) ran in Apps Script editor; JR eyeballed the converted PDF vs on-screen print preview, judged acceptable fidelity -- HTML->PDF survives @page breaks, mm units, table layout, borders, grayscale fills. Phase 4 smoke (group send + individual send) deferred to next session. Pitch RAINBOW-PITCH `0a289d2` rewrites Ripple.jsx page-3 footer per Sarvi feedback.

- [2026-05-05] **s065 -- audit Stage 3 triage clean + 1,290 smoke photos scrubbed + AskRainbow defensive client patch (commit `8a17e80` scheduling app + `3c05049` rainbow-pitch).** Resumed deferred `/audit` pipeline at Stage 3: Sonnet 4.6 subagent re-read every B1/B2 candidate from the s056 inventory against current source and confirmed each was already fixed by post-s056 commits (sickEvent extraction in `EmployeeView.jsx`, `.note` crash guard, four aria-label adds, button-type hardening). Verdict Ready to Merge; `B1: 0 | B2: 0 | Non-findings: 11`. Stage 6 dated report at `docs/audit-2026-05-04-deferred-resume.md`; triage at `.claude/skills/audit/output/triage.md`. `.gitignore` tightened to keep audit skill's regenerable runtime output (codebase-map, hunt-*.md, ~1MB) out of git while preserving triage.md + inventory.md as diff baselines. Smoke-photo scrub: 227 repo-root + 996 in `.playwright-mcp/` + 67 in `/tmp/` deleted (all gitignored, no source impact); tracked PNGs in `public/`/`pitchdeck/assets/`/`dist/` left intact. AskRainbow chatbot triage: endpoint healthy from curl across 5 prompts (longest 88.6s); defensive JSON patch on AskRainbow.jsx surfaces clearer errors.

- [2026-05-04] **Pitch deck revision shipped (s064; 13 commits 8b3996a..b71d79b on `~/APPS/RAINBOW-PITCH/`, fast-forwarded to main + pushed)** -- 10 plan-locked commits from coding-plan-executor plus 3 JR-driven follow-on (slide nav padding, Today.jsx annotation, Alternatives competitor copy). VC critique surfaced 11 objections; JR shipped with 2 soft spots deferred to TODO Blocked.

<!-- Older Completed entries trimmed per schema ">=5 most recent". Full history in git log. Recent trims: 2026-05-06 trim (drop hardcoded BCC + mobile onboarding z-index entries); s066 trim (Onboarding 6-attachments closed-not-a-bug 2026-05-04); s065 trim (Sadie cleanup s060 carry path 2 + 1c2f34c drop legacy deleteEmployee + 91b2976/a02f7c4/4e73a26/5c0d99c employee lifecycle redesign + 7fb3417/6850ca3/4f2cabf/8f6d204 past-period edit lock + Employees archive + Erase + aac976d/d86e949/dcfe87f/0c81705/535da13/ec62e8f desktop My Schedule modal + Mine migration + long-press instrumentation + c726379 desktop period-nav parity + e3090e4 Day Status renamed to Available + 48db3c4 login default-password hint); s060 trim (bcb12fd + 062bb22 + c817f07 + eb89e92 onboarding email modal + welcome PDF rebrand v2.31.0->v2.31.1 + state-refresh fix); s059 trim (23ad13b v2.30.2 batchSaveShifts uses withDocumentLock_; e53768e admin Day Status [Working/Sick/Unavailable] segmented control; 9ae5ed8 mobile long-press on multi-event cells opens EventDetailSheet; 33438e6 mobile violations triangle now opens panel); s058 trim (4a227d0 + 39c55e0 Audit fixes Batch 4 frontend session integrity + defensive guards); s057 trim (7abe364 v2.30.1 lock-timeout symmetry + 04f6912 Batch 3 TOCTOU + 1164c53 v2.29.1 changePassword case-fold + 31f1c23 Batch 2 type filter/cache bust/email lowercase); s056 trim (d18e72a Batch 1 audit fixes + adversarial-audit-plan-written + 6a17dcc violations panel + 8ab6a05 FirstnameL + 47a4ef4 eye-toggle); s055 trim (9376c7b mobile staff parity + Sheet boolean normalization, 3093c60 admin1 protection, f1efc38 self-lockout + owner-deactivation guard, 1a492ef 12h TimePicker, 576e50e EmailModal filter useMemo). -->
