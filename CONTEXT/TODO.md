...
- **[s068] PDF Export chunk-load resilience -- shipped, awaiting Sarvi smoke.** `3b5380e` swaps the auto-dismiss toast for a persistent modal with a Copy-to-clipboard button (also writes the error text to `localStorage['pdf-export-last-error']`). `d4a1bf7` adds a chunk-load detector in the catch arm: matches "Failed to fetch dynamically imported module" / "importing a module script failed" / "error loading dynamically imported module" and triggers `window.location.reload()` instead of showing the modal. Sarvi reported the original "long-ass error that disappears too fast" symptom on her desktop after Vercel auto-deploy of the sort fix; the chunk-load family is the only signal we have ("failed to fetch dynamically..." was all she could read). After her hard-refresh she should be on the new bundle; next click of Export PDF either silently reloads (chunk-load self-heal) or pops the modal with full error text. Smoke gate: JR confirms with Sarvi that Export PDF works post-refresh, OR she copies the modal text and we diagnose.

- **[s068] Schedule grid sort restored -- 4-bucket Sarvi/Admin/FT/PT shipped, smoked locally (`f58670d` + `b1c8ae0`).** Cursor's `db3e893` had collapsed the prior tier sort to a flat name-rank list, mixing admin2 staff into the alpha tail with employees. Fix uses a single internal `scheduleBucket` shared by sort + dividers; admin bucket combines `isAdmin or adminTier eq 'admin2' or isOwner` and applies `SCHEDULE_FIRST_NAME_RANK` then alpha; FT bucket = `employmentType eq 'full-time'` and not admin; PT bucket = everyone else. `employeeBucket` (Autofill modal) untouched, keeps its 5-value shape. Spelling alignment in `SCHEDULE_ROW_FIRST_NAME_ORDER`: `Daniel`->`Dan`, `Jessica`->`Jess`, `Anjali`->`Anejli` to match Sheet rows. Verified live grid via agent-browser: Sarvi -> `Joel, Dan, Genia, Scott, Jess, Domenica, Axl, Isha, Patrisha, Anejli, Ella, Feng` (admins) -> FT alpha -> PT alpha. Mobile + PDF surfaces inherit via shared bucket helpers; not separately smoked.
...
- **EmailModal v2 + email-format pass + residual checks.** PDF attach shipped + Phase 4 smoked s066 (`1ac1052` + v2.32.4). Next if touching email again: branded HTML spot-check; duplicate-email save -> live `DUPLICATE_EMAIL` reject (if not recently exercised).
...
- Missing validation: PDF Export chunk-load self-heal via Sarvi (`3b5380e` + `d4a1bf7`). Persistent error modal + auto-reload on `Failed to fetch dynamically imported module` family. Sarvi must hard-refresh once to pick up the new bundle, then retry Export PDF; either silently auto-recovers or shows modal with copy-able error text.
...
- [2026-05-06] **s066 -- EmailModal v2 PDF shipped + Phase 4 smoked (`1ac1052` + v2.32.4; pitch `0a289d2`).** Frontend: `buildScheduleHtml` extracted; EmailModal POSTs `pdfHtml` + `pdfFilename` on group + individual sends; App.jsx passes `events` + `timeOffRequests` for PDF badges/OFF cells. Backend: optional pdf attach via `Utilities.newBlob(html, 'text/html').getAs('application/pdf')`. Phase 0 probe + eyeball OK 2026-05-05. Phase 4 prod smoke 2026-05-06: group + individual sends PASS; Test Guy cleanup PASS. Repo `backend/pdf-probe.gs` removed post-smoke. Ripple.jsx p3 footer per Sarvi feedback.

- [2026-05-06] **Schedule UI -- co-owner on grid + never-list hidden strip (`74f78fc`, `7606c66`).** `filterSchedulableEmployees` no longer drops `isOwner`; floor vs off-floor is `showOnSchedule` for admin-tier rows. `SCHEDULE_UI_NEVER_LIST_EMAILS` in `constants.js` removes listed login emails from `hiddenStaff` below the grid.
