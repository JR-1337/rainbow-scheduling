...
- Per-day defaults decouple + availability widening -- next: execute approved plan `~/.claude/plans/per-day-defaults-are-humming-quokka.md` in a fresh chat. 7 steps (schema doc, Code.gs v2.24.0 + widener, frontend parse/save, Auto-Fill defaultShift-first, EmployeeFormModal new section, JR runs widener, verify). Decouples availability from Auto-Fill booked hours so widening availability for PK eligibility no longer changes Auto-Fill output. Approved 2026-04-19.
