...
- **Login screen: password show/hide eye toggle.** Raised s054 2026-05-02. Add the standard winky-eye icon (lucide `Eye` / `EyeOff`) inside the password input on `LoginScreen` so users can preview what they typed. Same affordance on ChangePasswordModal + EmployeeFormModal password field if they have raw inputs. Stateful per input (each field tracks its own visibility), default hidden. Small commit, ~10-15 lines.
- **First-login workflow + new initial-password pattern.** Raised s054 2026-05-02. Change the default password from `emp-XXX` (zero-padded row index) to `FirstnameL` -- first name + last initial (e.g. "John Richmond" -> `JohnR`, "Sarah O'Connor" -> `SarahO`). Touches: backend `saveEmployee` insert path (computes default from `name` field, writes to password column + hash/salt columns), admin "default password" display in EmployeeFormModal (preview the computed value before save), reference memory `reference_default_passwords.md` (pattern updated). Open Qs JR likely wants discussed: (a) collision handling -- two "John R" employees? Append a number? (b) name-parse edge cases -- hyphenated last names ("Smith-Jones" -> `JaneS`?), single-word "names" stored, missing last name? (c) onboarding email already-on-TODO above should include the computed initial password in the welcome body. (d) case sensitivity -- store as exact mixed case, allow case-insensitive match at login? Discuss before scoping.
...
- [2026-05-02] **Mobile staff parity + Sheet boolean normalization (commit `9376c7b`)** — JR's row showed up on mobile More -> Staff active list even with `isOwner=TRUE` in the Sheet. Two bugs in one commit: (1) `MobileStaffPanel.jsx:12` active filter was missing `!e.isOwner` (desktop EmployeesPanel.jsx:15 had it -- mobile/desktop parity miss). (2) `parseEmployeesFromApi` ([apiTransforms.js:27](src/utils/apiTransforms.js#L27)) spread raw API values without normalizing booleans. Sheet checkbox-typed cells return native bool, but plain-text "TRUE"/"FALSE" cells leak as strings -- any non-empty string is truthy in JS, so a text-typed `isOwner="FALSE"` would read as truthy and bypass owner-exclusion filters across half the codebase. Now normalizes 6 documented bool columns (active, isAdmin, isOwner, showOnSchedule, deleted, passwordChanged) at the API boundary. Build PASS. JR confirmed mobile staff list now hides him.

- [2026-05-02] **Admin1 protection (commit `3093c60`)** — Per JR ask: nobody can deactivate Sarvi (or any future admin1). Extends f1efc38's self/owner guard to all `isAdmin=TRUE` rows. Three layers: button disabled with tooltip in EmployeeFormModal, `saveEmployee` handler refuses, `deleteEmployee` handler refuses. Toast directs to "Demote to Staff first" if truly needed. Admin2 tier intentionally NOT protected (lower-privilege accounts). Build PASS.

- [2026-05-02] **Self-lockout + owner-deactivation guard (commit `f1efc38`)** — JR locked himself out by deactivating his own admin row mid-session. Three layers: EmployeeFormModal Set Inactive button disabled when editing self or owner, `saveEmployee` refuses with toast, `deleteEmployee` same shape. UI uses existing `isEditingSelf` flag (line 26) which already gated admin/delete controls. Build PASS. Backend Code.gs has no equivalent -- noted in commit body but JR confirmed he doesn't want a backend redeploy bundled.

- [2026-05-02] **12h TimePicker for store-hours day modal (commit `1a492ef`)** — JR's day-edit modal showed 24h time inputs (Chromebook locale). Only `<input type="time">` in the codebase was at `ColumnHeaderEditor.jsx:9-16`; rest of app uses canonical `TimePicker` primitive (Hr/Min/AM-PM selects, 6am-11pm range, 15-min granularity). Swapped local `TimeInput` for `TimePicker` from `./primitives`. Layout shifted Open/Close from inline side-by-side to stacked since each picker is wider. Internal value contract unchanged (HH:MM 24h string). Build PASS.

- [2026-05-02] **Batch 2: EmailModal filter useMemo (commit `576e50e`)** — Two filter chains at `EmailModal.jsx:12-16` (`emailableEmps`, `adminContacts`) ran on every render. Wrapped both in `useMemo([employees])`. 4-line change, build PASS. No smoke (modal renders infrequently). Re-rank dropped I-B (App.jsx isMounted guard) and F1 (MobileBottomNav inline arrow).
<!-- Older Completed entries trimmed per schema ">=5 most recent". Full history in git log. Recent trims: s054 trim (da8f89a a11y Escape on 4 modals, 07ad44f desktop period-nav a11y parity, ec0e962 audit B1 sickEvent + a11y, d13bc14 MobileAlertsSheet white-screen fix); s053 trim (8647947 sickEvent extraction across 3 paths, 0ff2c7d+0f396c3 audit Stage 3-7 full-sweep triage); s052 trim (f1425e7 email banner subtitle contrast); s051 trim (s050 paste-deploy verify, s049 audit skill v5, s049 notification body copy tightening); s050 trim (s045 BCC otr.scheduler, s045 OTR wordmark banner, s045 policy disclaimer). -->
