...
- **EmailModal v2 + email-format pass.** PDF attach shipped + Phase 4 smoked s066 (`1ac1052` + v2.32.4). s070 code-verification closed both residuals: branded HTML at `src/email/buildBrandedHtml.js` clean against the file's email-safe ruleset (charset declared, all-inline, table-only layout, brand-rule-compliant); duplicate-email guard symmetric front-to-back (`EmployeeFormModal.jsx:58-69` + `Code.gs:2368-2378`).
...
- **Optional: sweep "Sarvi's confirmed" -> "Sarvi's reported" -- partial done s070.** Auto-memory `project_otr_facts.md` + `MEMORY.md` index pointer swept (3 sites). Active CONTEXT files no longer carry the phrase (already swept in a prior session). Remaining: chatbot prompt in `~/APPS/RAINBOW-PITCH/` repo (not this repo); pick up next time RAINBOW-PITCH is open. Historical planning docs (`pitchdeck/build-plan.md`, `docs/research/scaling-migration-options-2026-04-26.md`) deliberately left untouched -- frozen snapshots.
