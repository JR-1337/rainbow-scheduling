...
- [2026-05-06] **saveEmployee privilege matrix v2.32.5 + frontend `d6010f4`.** Backend: explicit admin1 tier (`isOwner` or (`isAdmin` and `adminTier` neq `admin2`)) gates `isAdmin`/`adminTier` writes on non-owner targets; owner rows blocked for tier / `isOwner` / deactivate-down; no self tier edits; only owners may toggle `isOwner`. Frontend: `App.jsx` strips `isAdmin`/`adminTier` only when caller is not admin1 tier (fixes silent non-persist for non-owner admin1s); strips `isOwner` when caller is not owner. `EmployeeFormModal.jsx`: admin2 tier buttons disabled + tooltip. JR paste-deploy v2.32.5 confirmed.
