...
- **iPhone 12 / iOS Safari freeze investigation (Sarvi-reported).** Symptom: page visible but taps dead, exit + re-enter Safari tab fixes it, often after Save. Diagnosis points to synchronous render chain on Save (490-cell rebuild + `allViolations` recompute + `AnimatedNumber` rAF storm) blocking main thread on iPhone 12. Fix candidates ranked at `docs/iphone12-freeze-fixes-2026-05-06.md`. **Shipped:** Batch 1 A1+A2+A3 (`c386cb9`) -- useMemo week1/week2, EMPTY_EVENTS frozen constant, today hoist. Batch 2 A4 (`c83c1fe`) -- AnimatedNumber rAF chain dropped on hot paths. Batch 3 A5+B2+C3 -- useDeferredValue on shifts/events for allViolations (urgent renders skip the ~6,860-lookup recompute), Vite manualChunks split React vendor (95 kB gzip app chunk + 45 kB cached React chunk vs 140 kB combined before), PDF blob URL retention 10min->30s. **Remaining candidates:** A6 (memoize mobile grids, ~2-3hr), B1 (lazy 14 modals, ~1-2hr), B3 (mobile/desktop view split, bigger refactor), C2 (useToast cleanup, ~10min). C1 (drop modal blur) skipped per JR -- keep the visual. **Next:** wait for Sarvi feedback on whether the cumulative A1-A5 + B2 + C3 cures the freeze before chasing A6/B1.
