...
- [2026-04-30] **Apps Script CacheService for getAllData reads (commit `49b1053`)** — Wraps the 5 sheet reads in `getAllData` (Employees, Shifts, Settings, Announcements, ShiftChanges) with `CacheService.getScriptCache()`. 600s TTL, ~90KB chunk threshold (auto-splits oversized payloads), automatic invalidation via writer wrappers in `updateRow`/`updateCell`/`appendRow` plus 2 inline busts at the direct-delete sites in `saveShift` + `deleteAnnouncement`. Failure path falls through to live reads. Expected: cache-hit `getAllData` 7-8s -> 2-3s; cache-miss unchanged. Plan at `~/.claude/plans/jazzy-baking-sutherland.md`. Build PASS. **Manual step pending:** JR pastes `backend/Code.gs` into live Apps Script editor + redeploys (otr.scheduler@gmail.com). Verify via Executions tab for `cache HIT/MISS tab=` log lines + DevTools network timing on `/exec`. Migration retires this layer at cutover.

- [2026-04-30] **admin2 showOnSchedule parity with admin1 (commit `db7d6d1`)** — Five sites extended so admin2 (`adminTier='admin2'`, `isAdmin=false`) honors the same "Show on schedule grid?" toggle as admin1: form modal toggle visibility, `filterSchedulableEmployees`, EmailModal emailable list, CommunicationsPanel scheduledCount, App.jsx hiddenStaff. PDF page-split unchanged (admin2 stays on staff pages 1-2). Build PASS. JR also added missing `adminTier`/`title` headers to live Employees sheet (cols W+X) — saveEmployee was silently dropping the field via header-driven `updateRow`.
