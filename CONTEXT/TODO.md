...
- **Email logic + schedule distribution wiring (next-session priority)** -- new OTR Gmail captured: `otr.scheduler@gmail.com`. Audit complete: no hard-coded sender in `backend/Code.gs`; sender == Apps Script project owner. Migration plan written at `docs/email-sender-migration.md` (Path A re-own + re-deploy recommended; Path B GmailApp alias as fallback). Blocked on JR: transfer Sheet + Apps Script ownership to `otr.scheduler@gmail.com`, then I update `API_URL` in `src/utils/api.js` to the new `/exec` URL and smoke a test send.
- **In-app bug fixes (JR to enumerate next session if any)** -- s027 shipped 1 bug fix (mobile name column compression + card restructure); JR may have more.
