...
- Last validated: ScheduleCell memo (`feb094b`) + PDF lazy (`3cf6b09`) + ColumnHeaderCell extract / scheduledByDate (`1d0ccb1`) PASS s023 prod Playwright 2026-04-26 desktop 1400x900: PK cell in May 4-17 period (Sarvi Sat May 9) tap opened "Edit Shift" with Work/Meeting/PK tabs (memo intact); PDF Export button opened blob print tab "Rainbow Schedule - Week 19 & 20" (popup not blocked); column header `sat 9 11a-7p 0/20` rendered via O(1) scheduledByDate lookup (PK doesn't count toward staffing target -- correct).
...
- Last validated (partial): PDF Export tab + print preview (`8affd22`) + PDF legend MTG/PK/SICK (`0d3220e`) PASS s023 prod Playwright 2026-04-26 -- PDF blob tab opened, UTF-8 charset, no garbage glyphs, legend renders `MTG / Meeting / PK / Product Knowledge / SICK / Sick / ★ Has Task / OFF Approved Time Off` after role glyphs. Still missing: sick day clears meetings/PK in grid + sheet save (`e9772c8`); title field clear + space warning (`dd1a7c2`).
- Last validated: cell density `5f5f16f` PASS s023 prod Playwright 2026-04-26 -- desktop ScheduleCell + mobile both render `<span PK>` + `<span 10a-10:45a>` 2-row inline (no absolute event overlays); cell click opens Edit Shift correctly. Untested at scale: 3+ mixed-type rollup `MTG +N` pill (no live data with 3+ events on one cell at smoke time).
- Last validated: PKEventModal edit mode + Saturday quick-pick `78f02d7` PASS s023 prod Playwright 2026-04-26 -- booked Alex Fowler + Anasstasia PK on Sun Apr 26 (today) -> reopened modal in EDIT mode -> 2 booked checkboxes correctly checked, others unchecked, Save read `Save` disabled (no diff) -> deselected both -> Save read `Save (-2)` enabled (THE BUG IS FIXED on prod) -> Save persisted via batchSaveShifts atomically -> modal closed -> panel + cells reverted to clean state. Saturday quick-pick: untoggled = `📅` + gray-200; toggled = `✓` + filled OTR rotating accent (today=Sun=orange rgb(196,102,26)) + 2px box-shadow ring (rgba(196,102,26,0.2) 0 0 0 2px) + white text. Tap-again reverts cleanly.
- Last validated: bulk-clear PK from outside `63420ce` PASS s023 prod Playwright 2026-04-26 -- desktop Clear dropdown showed "PK by day" optgroup with `Sun, Apr 26 — 2 PK` after seeding; option value `__pk_day__:2026-04-26`; selecting it opened AutoPopulateConfirmModal with copy "Clear all 2 PK bookings on Sunday, April 26?". Mobile Clear sheet drilled into "PK BY DAY" sub-section showing `Sat, May 9 — 19 PK` (count = schedulableEmployees-only PK; admins/owner excluded by `daysWithPKInWeek` helper, by design and consistent with desktop).
- Partial validation: PDF logo gap `1d26daf` -- structural fix verified live on prod s023 2026-04-26 (PDF tab opens, header CSS is the new tightened set); ~20-27px delta vs prior iPad export STILL needs JR side-by-side eyeball on iPad print preview (Playwright viewport != iPad rendering, can't measure delta without baseline export).
- Last validated (admin paths): PKDetailsPanel `0fe138c` PASS s023 prod Playwright 2026-04-26 -- desktop admin grid (Apr 20-May 3 with seeded PK): rendered `PK THIS PERIOD / Sun, Apr 26 · 6p-8p · 2 booked: Alex Fowler, Anasstasia Trofimov`. Mobile admin Comms tab (May 4-17 with stale 22 PK): rendered `Sat, May 9 · 10a-10:45a · 22 booked: Alex Fowler, Anasstasia Trofimov, Dan Carman +19 more` (first-3 + +N truncation works). Returns null when no PK in period (verified before seeding). Untested: desktop employee grid + mobile employee alerts sheet (employee-side mounts use the same shared component).
