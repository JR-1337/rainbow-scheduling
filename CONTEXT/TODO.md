...
- **Email migration -- IN PROGRESS, partial state on prod** -- s030 progress: Sheet + Apps Script ownership transferred to `otr.scheduler@gmail.com`. New deployment URL `AKfycbznGQ...` live. `API_URL` in `src/utils/api.js:6` swapped + pushed at `8b64f4e`; bundle `index-C7ld0zvI.js` live on prod. SMOKE FAIL: emails still send from `johnrichmond007@gmail.com`. Root cause per 4-loop investigation in `docs/email-migration-investigation.md`: Apps Script versioned deployments are bound to original deployer's identity, not current script-project owner. Existing deployment ran as John under the hood despite UI panel showing "Execute as: Me (otr.scheduler)". Fix path documented in investigation doc Path A: archive existing deployment, in incognito as otr.scheduler run any function from script editor to force fresh OAuth grant, THEN deploy new -- this produces a new `/exec` URL that needs another `api.js` swap + push. Path B fallback (`GmailApp.sendEmail({from: alias})`) is one-line code change if Path A also fails. Walkthrough at `docs/email-migration-walkthrough.md` covers Parts 1-7 for non-technical execution.
