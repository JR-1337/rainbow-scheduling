...
- **Email upgrade (PDF auto-attached + branded body via MailApp)** -- NOW UNBLOCKED. Sender migration shipped 2026-04-27 s031 (`33afa4b`); auto-notifications now send from `otr.scheduler@gmail.com`. Next: convert `src/modals/EmailModal.jsx` from `mailto:` (which only opens the user's local Gmail and can't attach files) to backend POST that calls a new Apps Script action passing PDF as base64 + branded HTML body. Open design question ~2 days old: generate PDF in Apps Script vs accept frontend base64 blob. JR pain: button currently opens his personal Gmail with no PDF and no branded body.
